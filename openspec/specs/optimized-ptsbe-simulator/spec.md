# Optimized PTSBE Simulator

## Purpose

Implements the optimized Pre-sampled Trajectory Simulation with Batched Extraction (PTSBE) simulator. Uses Unified Path Variations (UPV) to compute a single contraction path across all pre-sampled error sets and Non-Degenerate Batched Sampling (NBS) to extract many unique bitstrings per contraction, achieving high-throughput data collection relative to traditional trajectory simulation.
