# Unoptimized PTSBE Simulator

## Purpose

Implements the unoptimized Pre-sampled Trajectory Simulation with Batched Extraction (PTSBE) simulator. Pre-samples all error sets before GPU work, computes separate contraction paths per error set (without UPV), and reuses those paths across all shots for each error set. Serves as an intermediate performance phase between traditional trajectory and optimized PTSBE.
