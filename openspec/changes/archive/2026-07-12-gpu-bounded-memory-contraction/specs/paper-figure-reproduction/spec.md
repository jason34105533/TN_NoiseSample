...
Figure and summary generation SHALL report the actual measured geometric-mean speedups alongside the paper's headline claims (~10⁸× non-proportional, ~10³× proportional) without altering, filtering, or cherry-picking results to match those claims. Any configuration that fails to complete (per the benchmarking-harness's success-tracking requirement) SHALL be visibly marked as such, not silently omitted from figures.
